package com.sundeinfo.sypglass.relay.task.relay64;

import com.sundeinfo.sypglass.relay.define.RelayKind;
import com.sundeinfo.core.job.AbstractTaskStarter;
import com.sundeinfo.sypglass.relay.mapper.oa.FormTableMain26Mapper;
...
public class RelayStarter extends AbstractTaskStarter<RelayStarter> {
    ...
    protected void start() {
        relayProcessService.updateProcessStatus(RelayKind.RELAY_64,2);//开始执行，状态更新为执行中
        logger.info(RelayKind.RELAY_64.description() + " 开始执行导入: " + formatter.format(new Date()));
    }

    @Override
    protected void end() {
        if (callBack.getResult() == 0){
            relayProcessService.updateProcessStatus(RelayKind.RELAY_64,1);//执行完毕，状态更新为执行正常
            logger.info(RelayKind.RELAY_64.description() + " 导入流程正常执行完毕");
        }else {
            relayProcessService.updateProcessStatus(RelayKind.RELAY_64,9);//执行完毕，状态更新为执行异常
            logger.info(RelayKind.RELAY_64.description() + " 导入流程执行异常");
        }
    }

    @Component("RelayFetchTask64")
    public class RelayFetchTask extends FetchExecute<RelayFetchTask> implements Task {
        @Autowired
        private FormTableMain26Mapper formTableMain26Mapper;

        @Override
        protected List<FormTableMain> findFormTableMainByRequestId(int requestid){
            return formTableMain26Mapper.selectByRequestId(requestid);
        }
        @Override
        protected RelayKind getRelayKind() {
            return RelayKind.RELAY_64;
        }
    }

    @Component("RelayExtractTask64")
