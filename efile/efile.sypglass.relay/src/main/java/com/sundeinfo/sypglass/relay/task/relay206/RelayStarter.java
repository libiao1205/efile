package com.sundeinfo.sypglass.relay.task.relay206;
...
public class RelayStarter extends AbstractTaskStarter<RelayStarter> {
    ...
    protected void start() {
        relayProcessService.updateProcessStatus(RelayKind.RELAY_206,2);//开始执行，状态更新为执行中
        logger.info(RelayKind.RELAY_206.description() + " 开始执行导入: " + formatter.format(new Date()));
    }

    @Override
    protected void end() {
        if (callBack.getResult() == 0){
            relayProcessService.updateProcessStatus(RelayKind.RELAY_206,1);//执行完毕，状态更新为执行正常
            logger.info(RelayKind.RELAY_206.description() + " 导入流程正常执行完毕");
        }else {
            relayProcessService.updateProcessStatus(RelayKind.RELAY_206,9);//执行完毕，状态更新为执行异常
            logger.info(RelayKind.RELAY_206.description() + " 导入流程执行异常");
        }
    }

    @Component("RelayFetchTask206")
    public class RelayFetchTask extends FetchExecute<RelayFetchTask> implements Task {
        ...
        protected RelayKind getRelayKind() {
            return RelayKind.RELAY_206;
        }
    }

    @Component("RelayExtractTask206")
