package com.sundeinfo.sypglass.relay.task.relay349;

import com.sundeinfo.sypglass.relay.define.RelayKind;
import com.sundeinfo.core.job.AbstractTaskStarter;
import com.sundeinfo.sypglass.relay.mapper.oa.FormTableMain38Mapper;
...
public class RelayStarter extends AbstractTaskStarter<RelayStarter> {
    ...
    protected void start() {
        relayProcessService.updateProcessStatus(RelayKind.RELAY_349,2);//开始执行，状态更新为执行中
        logger.info(RelayKind.RELAY_349.description() + " 开始执行导入: " + formatter.format(new Date()));
    }

    @Override
    protected void end() {
        if (callBack.getResult() == 0){
            relayProcessService.updateProcessStatus(RelayKind.RELAY_349,1);//执行完毕，状态更新为执行正常
            logger.info(RelayKind.RELAY_349.description() + " 导入流程正常执行完毕");
        }else {
            relayProcessService.updateProcessStatus(RelayKind.RELAY_349,9);//执行完毕，状态更新为执行异常
            logger.info(RelayKind.RELAY_349.description() + " 导入流程执行异常");
        }
    }

    @Component("RelayFetchTask349")
    public class RelayFetchTask extends FetchExecute<RelayFetchTask> implements Task {
        @Autowired
        private FormTableMain38Mapper formTableMain38Mapper;

        @Override
        protected List<FormTableMain> findFormTableMainByRequestId(int requestid){
            return formTableMain38Mapper.selectByRequestId(requestid);
        }
        @Override
        protected RelayKind getRelayKind() {
            return RelayKind.RELAY_349;
        }
    }

    @Component("RelayExtractTask349")
