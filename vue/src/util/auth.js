...
const authToken = {
    ...
    setToken: function(token){
        // TODO: 设置token，并填写有效期
        var maxAge = new Date(new Date().getTime() + 10000 * 1000)
        Cookies.set('token', token, {
            expires: maxAge
        })
    },
    // 设置是否密码超期枚举值
    setPasswordExceed: function(passwordExceed){
        // TODO: 设置token，并填写有效期
        var maxAge = new Date(new Date().getTime() + 10000 * 1000)
        Cookies.set('passwordExceed', passwordExceed, {
            expires: maxAge
        })
    },
    ...
    // 移除passwordExceed
    removePasswordExceed: function(){
        Cookies.remove('passwordExceed')
    },
